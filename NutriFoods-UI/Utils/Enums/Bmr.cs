...
{
    public static double BasalMetabolicRate(this Bmr bmr, BiologicalSexToken token, double weight, int height,
        int age) =>
        bmr == Bmr.None
            ? throw new ArgumentException($"Value {token} for gender is not recognized")
            : bmr.Formula(token, weight, height, age);

    public static double BasalMetabolicRate(this Bmr bmr, BiologicalSexes sex, int age, double weight, int height) =>
